...
impl <'a>Module<'a> {
	pub fn new() -> Module<'a> {
		Module {_name: "", _backend: TorchBackend {}, _params: HashMap::new(), 
	...
	fn _apply(&mut self, callback: fn(&mut Tensor))  {
		for (_, module) in self.modules_iter_mut() { module._apply(callback) }
		for param in self.params_iter_mut().filter_map(|(_, p)| {if p.valid() {Some(p)} else {None} }) {
			callback(param.data);
/*

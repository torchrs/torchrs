...
def wrap_function_decl(name, arguments):
    ...
    declaration += '\t' + 'fn ' + cname + \
        '(' + ', '.join(arg.name + ': ' + TYPE_TRANSFORMS[type].get(arg.type, arg.type) for arg in arguments[1:]) + ')'
    return declaration


def wrap_function_impl(arguments):
    cname = name
    type = 'Trait'
    declaration = ''
    declaration += '\t' + 'fn ' + cname + \
...
def wrap_backend_decl():
	...
	wrapper += "use tensor::{Tensor, TensorKind, THGenerator};\n\n"
	...
	for fn in nn_functions:
		wrapper += wrap_function_decl_rust(fn.name, fn.arguments) + ";\n"
	wrapper += "\n}"
...
def wrap_backend_impl_type(type):
	...
	wrapper += "use rutorch::*;\n\n"
	wrapper += "pub struct THNN_{}Backend {\n".format(type)
	wrapper += "\tstate: *mut ::std::os::raw::c_void,\n"
	wrapper += "}\n\n"
	wrapper += "impl BackendIntf for THNN_{}Backend {\n".format(type)
	for fn in nn_functions:
		wrapper += wrap_function_decl(fn.name, fn.arguments) + "{\n"
		wrapper += wrap_function_impl(fn.arguments)
		wrapper += "}\n"

	wrapper += "}\n"
	with open('src/nn/backends/thnn_{}.rs'.format(type.tolower())) as f:
		f.write(wrapper)

def wrap_backend_impls():
	for t in ['Float', 'Double']:
		wrap_function_impl_type(t)
